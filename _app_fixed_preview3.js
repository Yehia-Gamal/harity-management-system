...
async function applyRecoveryPassword_() {
  ...
  try {
    ...
    const msg = (e?.message || e?.error_description || '').toString().trim();
    if (hint) hint.textContent = msg ? `تعذر حفظ كلمة المرور: ${msg}` : 'ØªØ¹Ø°Ø± Ø­ÙØ¸ ÙƒÙ„Ù…Ø© Ø§Ù„Ù…Ø±ÙˆØ±';
  } finally {
    try { if (btn) btn.removeAttribute('disabled'); } catch { }
  }
}
...
function openCaseDetails(id, mode) {
  ...
  if (AppState.caseDetailsMode !== 'edit') {
    ...
    const medicalHtml = `<div style="grid-column:1/-1">${viewSection('Ø§Ù„Ø¬Ø§Ù†Ø¨ Ø§Ù„Ø·Ø¨ÙŠ', medicalCases.length
      ...
      : viewBox('Ù„Ø§ ØªÙˆØ¬Ø¯ Ø¨ÙŠØ§Ù†Ø§Øª Ø·Ø¨ÙŠØ©', 'â€”'))}\n</div>`;
...
function printCurrentCase() {
  ...
  w.document.write(`<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>Ø·Ø¨Ø§Ø¹Ø© Ø­Ø§Ù„Ø©</title>
    ...
    <div class="grid" style="margin-top:10px">
      <div class="box"><div><strong>Ø§Ù„Ø§Ø³Ù…:</strong> ${it.familyHead || ''}</div><div><strong>الهاتف:</strong> ${it.phone || ''}</div><div><strong>Ø§Ù„Ø¹Ù†ÙˆØ§Ù†:</strong> ${it.address || ''}</div></div>
      <div class="box"><div><strong>المحافظة:</strong> ${it.governorate || ''}</div><div><strong>Ø§Ù„Ù‚Ø±ÙŠØ©:</strong> ${it.area || ''}</div><div><strong>الفئة:</strong> ${it.category || ''}</div><div><strong>المستكشف:</strong> ${it.explorerName || ''}</div></div>
    </div>
    <div class="grid" style="margin-top:10px">
  ...
  w.document.close();
  w.focus();
  logAction('Ø·Ø¨Ø§Ø¹Ø© Ø­Ø§Ù„Ø©', it.id, 'تم فتح صفحة الطباعة');
}
...
async function renderUsersList() {
  ...
  try {
    ...
    host.innerHTML = list.map(p => {
      ...
      const badge = active ? '<span class="pill ok">مفعل</span>' : '<span class="pill off">Ù…Ø¹Ø·Ù„</span>';
...
async function addOrUpdateUser() {
  ...
  try {
    ...
    const parts = [];
    if (before.full_name !== name) parts.push(`الاسم: "${before.full_name}" â†’ "${name}"`);
