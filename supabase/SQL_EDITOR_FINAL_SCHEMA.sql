-- =========================================================
-- Decent Family / Khawatir Case Management
-- Final Supabase SQL Editor schema compatible with current frontend + Edge Functions
--
-- Safe to run repeatedly.
-- Includes:
-- - profiles / cases / audit_log
-- - email + last_seen_at + legacy_source_id compatibility columns
-- - automatic profile creation from auth.users
-- - RLS policies
-- - app permission helpers with role support
-- - users listing RPC
-- - profile admin RPCs
-- - safe case delete RPCs used by the frontend
-- - paginated cases/audit RPCs
-- =========================================================
...
-- Compatibility for older hosted projects.
...
-- Backfill profile email/username from auth.users where possible.
update public.profiles p
set
  email = coalesce(nullif(p.email, ''), u.email),
  username = coalesce(nullif(p.username, ''), nullif(split_part(coalesce(u.email, ''), '@', 1), ''), replace(p.id::text, '-', ''))
from auth.users u
where p.id = u.id
  and (p.email is null or p.email = '' or p.username is null or p.username = '');
...
create unique index if not exists profiles_legacy_source_id_key on public.profiles (legacy_source_id) where legacy_source_id is not null;
create index if not exists profiles_username_idx on public.profiles (username);
create index if not exists cases_updated_at_idx on public.cases (updated_at desc);
create index if not exists cases_data_governorate_idx on public.cases ((data ->> 'governorate'));
create index if not exists cases_data_area_idx on public.cases ((data ->> 'area'));
create index if not exists cases_data_case_grade_idx on public.cases ((data ->> 'caseGrade'));
...
-- =========================
-- 4) AUTO-CREATE / SYNC PROFILE FROM AUTH
-- =========================
...
-- Backward compatibility for older policies/queries that still reference has_permission(text).
...
set search_path = public
as $$
  select public.has_app_permission(p)
$$;
...
-- Prevent direct self-escalation through table update.
create or replace function public.enforce_profiles_write_rules()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  -- Service-role / Edge admin calls have no user JWT in auth.uid(); allow them.
  if auth.uid() is null then
    return new;
  end if;

  if new.id <> auth.uid() and not public.has_app_permission('users_manage') then
...
drop trigger if exists trg_profiles_enforce on public.profiles;
create trigger trg_profiles_enforce
before update on public.profiles
for each row
execute function public.enforce_profiles_write_rules();

-- =========================
-- 6) RPCS USED BY THE APP
-- =========================
drop function if exists public.list_profiles_public();
drop function if exists public.delete_case(text);
drop function if exists public.delete_case(text, text);
drop function if exists public.delete_all_cases();
drop function if exists public.delete_all_cases(text, text);
...
begin
  ...
  select
    p.id,
    case when public.has_app_permission('users_manage') then p.email else null end as email,
    p.username,
    p.full_name,
    case
...
begin
  ...
  update public.profiles p
  set is_active = coalesce(p_is_active, p.is_active), updated_at = now()
  where p.username = v_username;
...
begin
  ...
  insert into public.audit_log(action, case_id, details, created_by)
  values ('حذف جميع الحالات', '', 'delete_all_cases RPC | deleted: ' || v_deleted::text, auth.uid());
...
drop policy if exists profiles_update_self_or_manage on public.profiles;
...
drop policy if exists cases_select_by_perm on public.cases;
...
drop policy if exists cases_insert_by_perm on public.cases;
...
drop policy if exists cases_update_by_perm on public.cases;
...
drop policy if exists cases_delete_none on public.cases;
drop policy if exists cases_delete_none_anon on public.cases;
create policy cases_delete_none
on public.cases
for delete
to authenticated
using (false);
...
drop policy if exists audit_select_by_perm on public.audit_log;
